...
def inp2img(inp):
    """
    I'm not really using this function anymore
    """
    inp = np.array(inp)
...
for idx in tqdm(range(0, 10), position=0, leave=True): 
    ...
    if t.sameInShape and t.sameOutShape:
        if all(t.trainSamples[0].inMatrix.shape == s.inMatrix.shape for s in t.testSamples):       
            ...
            score = [incorrectPixels(pred, s.outMatrix.m) for s in t.testSamples]
        ...
        with torch.no_grad():
            for s in t.testSamples:
                ...
                for mm in models:
                    ...
                    score.append(incorrectPixels(x, t.testSamples[0].outMatrix.m))
                pred = x
                predictions.append(pred)
            score = [incorrectPixels(predictions[i], t.testSamples[i].outMatrix.m) for i in range(t.nTest)]
    ...
    if t.sameInShape and t.sameOutShape:
        ...
        with torch.no_grad():
            for s in t.testSamples:
                ...
                scores.append((i, incorrectPixels(pred, s.outMatrix.m)))
            ...
            for mtt in modelsToTry:
                ...
                with torch.no_grad():
                    ...
                    for iS in range(len(inTrain)):
                        ...
                        sc += incorrectPixels(x, outTrain[iS])
...
if t.sameInShape and t.sameOutShape:
    ...
    with torch.no_grad():
        for s in t.testSamples:
            ...
            scores.append((i, Utils.incorrectPixels(pred, s.outMatrix.m)))
            predictions.append((i, pred))
        ...
        if isGood:
            ...
            for e in range(num_epochs):
                ...
                for s,label in trainShapes:
                    ...
                    inFeatures[t.nColors] = s.nPixels
                ...
                for s in t.testSamples:
                    ...
                    for shape in s.inMatrix.shapes:
                        if shape.color in t.colors:
                            ...
                            inFeatures[t.nColors] = shape.nPixels
